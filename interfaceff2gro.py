...
"""
modified by Yujiu Liu
    1. work fine on windows system
    2. reformat code
"""
...
def atomtypes_from_ff(l):
    '''Extracts atomtypes from .prm charmm forcefield file. l is argument being a list containing the lines of
    forcefield file. '''
    f, i, s = float, int, str
    g = []
...
def bonds_from_ff(l):
    '''Extracts bondtypes from .prm charmm forcefield file. l is argument being a list containing the lines of
    forcefield file. '''
    f, i, s = float, int, str
    g = []
    for m in range(len(l)):
...
def angles_from_ff(l):
    '''Extracts angletypes from .prm charmm forcefield file. l is argument being a list containing the lines of
    forcefield file. '''
    f, i, s = float, int, str
    g = []
    for m in range(len(l)):
...
def create_lammps_data_file():
    ...
    bash_command = os.getcwd() + '/msi2lmp/src/msi2lmp.exe ' + \
                   sys.argv[1][:-4] + ' -class 2 -frc ' + os.getcwd() + '/msi2lmp/frc_files/pcff_interface.frc -ignore'
    g = subprocess.Popen(bash_command, shell=True, stdin=subprocess.PIPE, stdout=subprocess.PIPE,
                         stderr=subprocess.STDOUT)
    print(g.stdout.read().decode('utf8'))


def clean_lammps_data_file():
    '''Removes .data file created by the lammps utility software '''
    datafile = sys.argv[1][:-4] + '.data'
    print('Cleaning temporary file ' + datafile + '\n')
    os.remove(os.getcwd() + '/' + datafile)
...
try:
    ...
    print(
        'First command-line argument is not a proper .car file, or .mdf file is missing/having different name than '
        'the .car file, or msi2lmp.exe is not located in ' + os.path.dirname(
            sys.argv[0]) + '/msi2lmp/src.')
...
try:
    forcefield = open(os.getcwd() + '/forcefield/charmm27_interface_v1_5.prm', 'r').readlines()
except:
...
element2atomNumber = {'H': '1', 'He': '2', 'C': '6', 'N': '7', 'O': '8', 'F': '9', 'Ne': '10', 'Na': '11', 'Mg': '12',
                      'Al': '13', 'P': '15', 'S': '16', 'Cl': '17', 'K': '19', 'Ca': '20', 'Fe': '26', 'Zn': '30',
                      'Br': '35', 'I': '53', 'Cs': '55', 'Si': '14', 'Ag': '47', 'Au': '79', 'Cu': '29', 'Ni': '28',
                      'Pb': '82', 'Pd': '46', 'Pt': '78'}

# Dictionary for masses
mass2element = {1.007970: 'H', 1.008000: 'H', 15.999400: 'O', 40.080000: 'Ca', 22.989770: 'Na', 26.981530: 'Al',
                26.982000: 'Al', 28.086000: 'Si', 30.973800: 'P', 39.098300: 'K', 107.868000: 'Ag', 196.967000: 'Au',
                63.546000: 'Cu', 58.710000: 'Ni', 207.200000: 'Pb', 106.400000: 'Pd', 195.090000: 'Pt'}
...
for m in atomtypes_universe:
    if m[1] > 10:
        itp_file.write(cformat_atomtypes % (
            atom_lammps_dict[int(m[0])], element2atomNumber[mass2element[m[1]]], m[1], 0, 'A',
            2.0 ** (-1.0 / 6.0) * 0.2 * atom_ff_sigma[atom_lammps_dict[int(m[0])]],
            -kcal2kJ * atom_ff_epsilion[atom_lammps_dict[int(m[0])]]))
    else:
        itp_file.write(cformat_atomtypes_2 % (
            atom_lammps_dict[int(m[0])], element2atomNumber[mass2element[m[1]]], m[1], 0, 'A',
            2.0 ** (-1.0 / 6.0) * 0.2 * atom_ff_sigma[atom_lammps_dict[int(m[0])]],
            -kcal2kJ * atom_ff_epsilion[atom_lammps_dict[int(m[0])]]))
...
for m in atoms_universe:
    itp_file.write(
        cformat_atoms % (m[0] + 1, atom_lammps_dict[int(m[1])], 1, 'SURF', mass2element[m[2]], m[0] + 1, m[3], m[4]))
